  ...
  return (
    <>
      ...
      <button
        type="button"
        onClick={() => start().catch(() => {})}
        className="fixed bottom-4 right-4 z-50 rounded-full bg-orange-500 px-4 py-2 text-sm font-semibold text-white shadow-lg transition hover:bg-orange-400 focus:outline-none focus:ring-2 focus:ring-orange-300 focus:ring-offset-2 focus:ring-offset-slate-900 flex items-center justify-center"
      >
        <span className="text-center">{isPlaying ? 'Music playing' : 'Play music'}</span>
      </button>
    </>
  )
}
